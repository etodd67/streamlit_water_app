...
DATA_URL = "~/Desktop/Harbor_Water_Quality3.csv"
...
normalized_column = (selected_data[selected_column] - selected_data[selected_column].min()) / (selected_data[selected_column].max() - selected_data[selected_column].min())
...
hexagon_layer = pdk.Layer(
    ...
    color_range_steps=6,
    get_fill_color=f"[255 * {selected_column}_normalized, 255 * (1 - {selected_column}_normalized), 0, 200]",  # Gradient from yellow to red based on normalized value
)
...
smooth_yearly_median_values = yearly_median_values.interpolate(method='linear')
...
with open("app.py", "r") as script_file:
